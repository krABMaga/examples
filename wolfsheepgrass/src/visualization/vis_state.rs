use crate::{GAIN_ENERGY, INIT_ENERGY, WOLF_REPR, model::{
    animals::Animal,
    grass::{FULL_GROWN},
    state::State,
}};
...
use rust_ab::{bevy::prelude::{Commands, ResMut}, rand::prelude::ThreadRng};
...
impl OnStateInit<Animal> for VisState {
    fn on_init(
        ...
        Self::generate_grass(&state, &mut sprite_render_factory, &mut commands, &mut sim, &mut rng);
