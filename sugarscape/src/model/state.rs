...
impl Patch{
    pub fn new(id:u32, sugar_amount:u32, sugar_growback:u32) -> Self{
        Patch{id, sugar_amount, sugar_growback:1}
    }
}
...
impl State for Environment {
    ...
    fn init(&mut self, schedule: &mut Schedule) {
        ...
        let bottom_left_mid = ((self.dim.0 as f32*0.25) as i32, (self.dim.1 as f32*0.25)as i32);
        let up_right_mid = ((self.dim.0 as f32*0.75) as i32 -1, (self.dim.1 as f32*0.75)as i32);
        ...
        for i in 0..self.dim.0
        {
            for j in 0..self.dim.1
            {
                ...
                let sumsq_corner_down= x_corner_down*x_corner_down + y_corner_down*y_corner_down;

                let mut sugar_amount=1;

                if((0 <= sumsq) && (sumsq <= dim_div)) || ((0 <= sumsq_up) && (sumsq_up <= dim_div))
                {
                    sugar_amount=3
                }
                else if ((0 <= sumsq) && (sumsq <= dim_div*2)) || ((0 <= sumsq_up) && (sumsq_up <= dim_div*2)){
                    sugar_amount=2;
                }
                else if (0 <= sumsq_corner) && (sumsq_corner <= dim_div) || (0 <= sumsq_corner_down) && (sumsq_corner_down <= dim_div){
                    sugar_amount=0;
                }
        ...
        for i in 0..self.num_agents {
            ...
            let agent=Eater{id: i, position: pos, vision: rng.gen_range(1..4),metabolism: rng.gen_range(1..4),age: 0,max_age: 20,wealth: 20};
