...
impl AgentRender for BirdVis {
    ...
    fn location(&self, agent: &Box<dyn Agent>, state: &Box<&dyn State>) -> (f32, f32, f32) {
        ...
        match loc {
            ...
            None => (agent.loc.x as f32, agent.loc.y as f32, agent.loc.z as f32),
        }
    }
    ...
    fn rotation(&self, agent: &Box<dyn Agent>, _state: &Box<&dyn State>) -> f32 {
        ...
        let rotation = if concrete_agent.last_d.x == 0. || concrete_agent.last_d.y == 0. || concrete_agent.last_d.z == 0. {
            0.
        } else {
            //calculate the rotation of the bird in 3d
    ...
    fn update(
        ...
        let (loc_x, loc_y, loc_z) = self.location(agent, state);
